...
if __name__ == "__main__":
	...
	arena = finite_arena(11)

	arena.addPreds(0,[6,7])
	arena.addPreds(1,[3,4])
	arena.addPreds(2,[1,3])
	arena.addPreds(3,[4,5])
	arena.addPreds(4,[8])
	arena.addPreds(5,[2,4,8])
	arena.addPreds(6,[0,7])
	arena.addPreds(7,[8])
	arena.addPreds(8,[5,9])
	arena.addPreds(9,[8,10])
	arena.addPreds(10,[8,6])

	arena.setPlayers([0,0,0,1,1,0,0,1,1,0,1])

	print(arena.attractor([0,1,2], 0))

	# arena = finite_arena(10)
	# arena.addPreds(0,[7,9])
	# arena.addPreds(1,[3])
	# arena.addPreds(2,[3,5])
	# arena.addPreds(3,[4])
	# arena.addPreds(4,[1,5,6])
	# arena.addPreds(5,[9])
	# arena.addPreds(6,[4,5,8,9])
	# arena.addPreds(7,[6])
	# arena.addPreds(8,[0])
	# arena.addPreds(9,[7,8])

	# arena.setPlayers([0,0,0,1,1,0,1,0,0,1])
