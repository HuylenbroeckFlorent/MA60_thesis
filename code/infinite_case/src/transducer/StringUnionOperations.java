...
final public class StringUnionOperations {
	 ...
	 */
	private static transducer.State convert(State s, 
			IdentityHashMap<State, transducer.State> visited) {
		transducer.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new transducer.State();
	 ...
	 */
	public static transducer.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, transducer.State>());
	}
