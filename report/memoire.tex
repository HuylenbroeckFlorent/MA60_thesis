...
\noindent Une stratégie pour le joueur $j_n$ est gagnante \emph{à partir d'un sommet} $v$ si une partie jouée selon cette stratégie mène à une victoire du joueur $j_n$.
...
\end{defi}
Cette nouvelle définition de l'ensemble gagnant pour un jeu de sûreté nous donne immédiatement une stratégie gagnante pour le joueur $j_0$. Etant donné que $I\in W_0$, celui-ci n'a qu'à déplacer le pion vers un sommet de $W_0$ à chaque coup.\\
...
\Repeat
	...
	\If{$counterexample\neq \emptyset$}
		...
		\If{$counterexample$ est un contre-exemple positif $u$}
			\State $PrefixTree.add(u, 1)$
		\ElsIf{$counterexample$ est un contre-exemple négatif $u$}
			\State $PrefixTree.add(u, -1)$
		\Else
...
\begin{algorithm}[H]
\begin{algorithmic}[1]
\algrestore{construct}
\For{$q$ allant de $0$ à $n-1$}
	\State $f[q]\gets nVar$
	\State $nVar\gets nVar+1$
\EndFor
\For{$p$ allant de $0$ à $n-1$}
	\For{chaque $a\in\Sigma$}
		\For{$q$ allant de $0$ à $n-1$}
	...
	\EndFor
\EndFor
\For{$u$ allant de $0$ à $|prefixTree|-1$}
	\For{$q$ allant de $0$ à $n-1$}
...
\While{$|prefixes|>0$}
	...
	\EndFor
	\For{chaque $a\in\Sigma$}
		\If{$prefix$ à un successeur pour $a$}
...
\If{$|S.Uni|>0$}
	\For{$u$ allant de $0$ à $|S.Uni|$}
		...
		\For{$p'$ allant de $0$ à $|aStates|-1$}
			\State $aState\gets aStates[p']$
			\For{chaque transition $\delta^\mathcal{A}(aState,a)=aState2$ de $\mathcal{A}$}
...
\algrestore{construct}
		\For{$p'$ allant de $0$ à $|aStates|-1$}
			\State $aState\gets aStates[p']$
			\For{chaque transition $d(aState,a)=aState2$ de $\mathcal{A}$}
				\For{$p$ allant de $0$ à $n-1$}
					\For{$q$ allant de $0$ à $n-1$}
						\State $q'\gets$ indice de $aState2$ dans $aStates$
						\For{$l$ allant de $0$ à $l_{max}-2$}
			...
			\EndFor
			\For{$q$ allant de $0$ à $n-1$}
				\For{$l$ allant de $1$ à $l_{max}-1$}
					\State $reverse\gets faux$
					\For{$p2'$ allant de $0$ à $|aStates|-1$}
						\State $aState2\gets aStates[p2']$
						\For{chaque transition $\delta^\mathcal{A}(aState2,a)=aState$ de $\mathcal{A}$}
							\For{$p$ allant de $0$ à $n-1$}
...
\end{itemize}
La dernière étape pour ce calcul de complexité est de se demander combien de fois au maximum la boucle principale va s'exécuter. Pour un nombre d'états $n$ et un alphabet de taille $a$, il y a au plus $an!$ configurations d'AFD. Ainsi, dans le pire des cas, la boucle principale s'exécutera $an!$ fois. \\
...
\node (1) [state, initial, accepting, initial text=] {$0$};
\end{tikzpicture}
\end{figure}
...
\node (1) [state, initial, initial text=] {$0$};
\node (2) [state, right of=1] {$1$};
\node (3) [state, right of=2] {$2$};
\node (4) [state, accepting, right of=3] {$3$};

\path[->]
	(1) edge node {$s$}   (2)
	(2) edge node {$\unary$}   (3)
	(3) edge node {$\unary$}   (4)
	(4) edge [loop right] node {$\unary$} ();
...
\node (4) [state, accepting, right of=3] {$4$};
\node (5) [state, below right of=1] {$5$};

\path[->]
	(0) edge node[anchor=north] {$s$}   (5)
	(0) edge node {$e$}   (1)
	(1) edge node {$\unary$}   (2)
	(2) edge node {$\unary$}   (3)
	(3) edge node {$\unary$}   (4)
	(4) edge [loop right] node {$\unary$} ()
	(5) edge node[anchor=north]  {$\unary$}   (3);
