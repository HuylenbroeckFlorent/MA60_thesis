...
\end{equation}
qui assure qu'un unique état $q$ peut être atteint depuis un sommet $p$ en lisant le mot $a$. Ainsi, il est possible pour l'élève de construire un AFD $\mathcal{A}_\mathfrak{M}$ à partir d'un modèle $\mathfrak{M}$ satisfaisant $\phi_n^{AFD}=(\ref{booldeter1})$.
\begin{defi}
Soit $\mathfrak{M}$ un modèle satisfaisant $\phi_n=(\ref{booldeter1})$, alors on peut construire $\mathcal{A}_\mathfrak{M}=(Q,\Sigma,\delta,q_0,F)$ avec $Q$ et $q_0$ fixés, $\Sigma$ annoncé par l'enseignant, $\delta(p,a)=q$ pour tout $\mathfrak{M}(d_{p,a,q})=$ vrai, et $F=\{q\in Q$ $|$ $\mathfrak{M}(f_q)=$ vrai $\}$
\end{defi}
\noindent Il reste maintenant à rendre cet AFD cohérent avec $S$. Pour cela, il est nécessaire que $\mathcal{A}_\mathfrak{M}$ accepte ou rejette tous les mots de $S$ définis par l'ensemble $W=Pos\cup Neg\cup Ante(Ex)$ $\cup$ $Ante(Uni)$ (où, pour un contre-exemple d'implication $X=(u,\mathcal{A})$, on a $Ante(X)=u$). Nous allons donc introduire une nouvelle variable boléenne $x_{u,q}$, pour laquelle $x_{u,q}=$ vrai signifie que $\mathcal{A}_\mathfrak{M}$ atteint l'état $q\in Q$ en lisant le mot $u\in Pref(W)$.\\
...
\end{equation}
laquelle indique que n'importe quel parcours de $\mathcal{A}_\mathfrak{M}$ commence par l'état $q_0$ et la formule
\begin{equation}
\label{boolWtotal}
\begin{aligned} \bigwedge _{p \in Q}~ \bigwedge _{a \in \varSigma }~ \bigvee _{q \in Q}~ d_{p, a, q} \end{aligned}
\end{equation}
qui indique que la fonction transition est totale, autrement dit que $\forall p\in Q, a\in\Sigma, \exists q\in Q, \delta(p,a)=q$. Cette condition est nécessaire dans notre objectif de produire un AFD $\mathcal{A}_\mathfrak{M}$ qui encode tous les préfixes des mots de $W$.\\

\noindent Ensuite, la formule suivante
\begin{equation}
\label{boolW2}
\begin{aligned} \bigwedge _{u \in Pref (W)}~ \bigwedge _{q \ne q' \in Q}~ \lnot x_{u, q} \vee \lnot x_{u,q'} \end{aligned}
\end{equation}
assure que pour chaque mot $u$ de l'ensemble des préfixe des mots de l'échantillon $S$, il existe au plus un état $q$ tel que $x_{u,q}=$ vrai.
Et la formule
\begin{equation}
\label{boolW3}
\begin{aligned} \bigwedge _{ua \in Pref (W)}~ \bigwedge _{p,q \in Q}~ (x_{u, p} \wedge d_{p, a, q}) \rightarrow x_{ua, q} \end{aligned}
\end{equation}
décris la manière dont un parcours de l'automate $\mathcal{A}_\mathfrak{M}$ par un mot $ua\in Pref(W)$ fonctionne. Si un état $p$ est atteint en lisant le mot $u$, et qu'une transition $\delta(p,a)=q$ existe (i.e. la variable $d_{p,a,q}=$ vraie) alors l'état $q$ sera atteint en lisant le mot $ua$.\\

\noindent La conjonction de ces quatre formules $\phi^W_n=(\ref{boolW1})\land(\ref{boolWtotal})\land(\ref{boolW2})\land(\ref{boolW3})$ nous permet ainsi de produire un AFD $\mathcal{A}_\mathfrak{M}$ qui permettra d'accepter ou de rejetter tous les mots de $W$.\\
...
\end{equation} 
assurera, par un raisonnement similaire, que les mots de $Neg$ seront rejetés par $\mathcal{A}_\mathfrak{M}$, et donc que $\mathcal{L}(\mathcal{A}_\mathfrak{M})\cap Neg=\emptyset$.
